...
def _estimate_initial_velocity(history_xyz: torch.Tensor, history_rot: torch.Tensor, dt: float) -> torch.Tensor:
    ...
    *leading, intervals, _ = dxy.shape
    ...
    velocity = torch.linalg.solve(lhs, rhs)
    return velocity[..., -1]
...
class UnicycleTrajectoryDecoder(nn.Module):
    ...
    def __init__(self, config: dict) -> None:
        ...
        self.register_buffer("accel_mean", torch.tensor(config.get("accel_mean", 0.0)), persistent=False)
        self.register_buffer("accel_std", torch.tensor(config.get("accel_std", 1.0)), persistent=False)
        self.register_buffer("curvature_mean", torch.tensor(config.get("curvature_mean", 0.0)), persistent=False)
        self.register_buffer("curvature_std", torch.tensor(config.get("curvature_std", 1.0)), persistent=False)
