...
class RequestSpecDecodeMetrics:
    ...
    vLLM 0.28 removed its per-request accumulator while retaining aggregate
    server metrics. Omni needs the request-scoped snapshot for structured
    model APIs, so it owns that small compatibility surface explicitly.
    """
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        num_accepted = sum(
            accepted * count for accepted, count in enumerate(self.histogram)
        )
        mean_acceptance_length = (
            1.0 + num_accepted / num_spec_steps if num_spec_steps else 1.0
        )
        draft_acceptance_rate = (
            num_accepted / self.num_draft_tokens if self.num_draft_tokens else 0.0
        )
        result: dict[str, Any] = {
