...
def main() -> None:
    args = parse_args()
    image_paths = [
        args.sample_dir / "images" / f"{camera}_t{frame}.jpg"
    ...
    try:
        for run_index in range(args.runs):
            ...
            for image_index, image in enumerate(images):
                channel = image_index % int(image.shape[0])
                image[channel, 0, 0] = (int(image[channel, 0, 0]) + run_index + image_index + 1) % 256
            ...
            outputs = omni.generate("Drive forward.", sampling_params_list=[sampling])
